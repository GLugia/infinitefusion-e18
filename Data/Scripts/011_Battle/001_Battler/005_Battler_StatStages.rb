class PokeBattle_Battler
  ...
  def pbCanLowerStatStage?(stat, user = nil, move = nil, showFailMsg = false, ignoreContrary = false)
    ...
    if !user || user.index != @index   # Not self-inflicted
      ...
      if abilityActive?
        return false if BattleHandlers.triggerStatLossImmunityAbility(@ability, self, stat, @battle, showFailMsg) if !@battle.moldBreaker
  ...
  def pbLowerAttackStatStageIntimidate(user)
    ...
    if !hasActiveAbility?(:CONTRARY)
      ...
      if abilityActive?
        if BattleHandlers.triggerStatLossImmunityAbility(@ability, self, :ATTACK, @battle, true) ||
           BattleHandlers.triggerStatLossImmunityAbilityNonIgnorable(@ability, self, :ATTACK, @battle, true)
