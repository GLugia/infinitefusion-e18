...
BattleHandlers::AbilityOnStatLoss.add(:COMPETITIVE,
  proc { |ability, battler, stat, user|
    ...
    battler.pbRaiseStatStageByAbility(:SPECIAL_ATTACK, 2, battler, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::AbilityOnStatLoss.add(:DEFIANT,
  proc { |ability, battler, stat, user|
    next if user && !user.opposes?(battler)
    battler.pbRaiseStatStageByAbility(:ATTACK, 2, battler, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::AbilityOnFlinch.add(:STEADFAST,
  proc { |ability, battler, battle|
    battler.pbRaiseStatStageByAbility(:SPEED, 1, battler, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::TargetAbilityOnHit.add(:JUSTIFIED,
  proc { |ability, user, target, move, battle|
    ...
    target.pbRaiseStatStageByAbility(:ATTACK, 1, target, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::TargetAbilityOnHit.add(:RATTLED,
  proc { |ability, user, target, move, battle|
    ...
    target.pbRaiseStatStageByAbility(:SPEED, 1, target, true, GameData::Ability.get(ability).real_name)
  }
)

BattleHandlers::TargetAbilityOnHit.add(:STAMINA,
  proc { |ability, user, target, move, battle|
    target.pbRaiseStatStageByAbility(:DEFENSE, 1, target, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::TargetAbilityOnHit.add(:WATERCOMPACTION,
  proc { |ability, user, target, move, battle|
    ...
    target.pbRaiseStatStageByAbility(:DEFENSE, 2, target, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::TargetAbilityOnHit.add(:WEAKARMOR,
  proc { |ability, user, target, move, battle|
    ...
    target.pbRaiseStatStageByAbility(:SPEED, speed_stages, target, false, GameData::Ability.get(ability).real_name)
    battle.pbHideAbilitySplash(target)
  }
)
...
BattleHandlers::UserAbilityEndOfMove.add(:BEASTBOOST,
  proc { |ability, user, targets, move, battle|
    ...
    highest_stat = user.plainStats.each_with_index.max
    
    stat_id = GameData::Stat[highest_stat[1]]
    
    if user.pbCanRaiseStatStage?(stat_id, user)
      user.pbRaiseStatStageByAbility(stat_id, numFainted, user, true, GameData::Ability.get(ability).real_name)
    end
  }
)
...
BattleHandlers::UserAbilityEndOfMove.add(:MOXIE,
  proc { |ability, user, targets, move, battle|
    ...
    user.pbRaiseStatStageByAbility(:ATTACK, numFainted, user, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::TargetAbilityAfterMoveUse.add(:BERSERK,
  proc { |ability, target, user, move, switched, battle|
    ...
    target.pbRaiseStatStageByAbility(:SPECIAL_ATTACK, 1, target, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::EOREffectAbility.add(:MOODY,
  proc { |ability, battler, battle|
    ...
    if randomDown.length > 0
      ...
      battler.pbLowerStatStageByAbility(randomDown[r], 1, battler, false, false, GameData::Ability.get(ability).real_name)
    end
    battle.pbHideAbilitySplash(battler)
...
BattleHandlers::AbilityOnSwitchIn.add(:DOWNLOAD,
  proc { |ability, battler, battle|
    ...
    battler.pbRaiseStatStageByAbility(stat, 1, battler, true, GameData::Ability.get(ability).real_name)
  }
)
...
BattleHandlers::AbilityOnBattlerFainting.add(:SOULHEART,
  proc { |ability, battler, fainted, battle|
    battler.pbRaiseStatStageByAbility(:SPECIAL_ATTACK, 1, battler, true, GameData::Ability.get(ability).real_name)
  }
)
