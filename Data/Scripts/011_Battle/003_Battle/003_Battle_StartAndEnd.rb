class PokeBattle_Battle
  ...
  def pbEndOfBattle
    ...
    pbGainMoney if @decision == 4
    # Pass on Pokérus within the party and fusion parts
    if @internalBattle
      infected = []
      ...
      infected.each do |idxParty|
        ...
        # pass to host's parts
        if $Trainer.party[idxParty].fused
          if $Trainer.party[idxParty].head_pokerus == 0
            $Trainer.party[idxParty].head_pokerus = $Trainer.party[idxParty].pokerus if rand(3) == 0
          end
          if $Trainer.party[idxParty].body_pokerus == 0
            $Trainer.party[idxParty].body_pokerus = $Trainer.party[idxParty].pokerus if rand(3) == 0
          end
        end
        if idxParty > 0 && $Trainer.party[idxParty - 1].pokerusStage == 0
          $Trainer.party[idxParty - 1].givePokerus(strain) if rand(3) == 0   # 33%
          # pass to neighbor's parts
          if $Trainer.party[idxParty - 1].fused
            if $Trainer.party[idxParty - 1].head_pokerus == 0
              $Trainer.party[idxParty - 1].head_pokerus = $Trainer.party[idxParty - 1].pokerus if rand(3) == 0
            end
            if $Trainer.party[idxParty - 1].body_pokerus == 0
              $Trainer.party[idxParty - 1].body_pokerus = $Trainer.party[idxParty - 1].pokerus if rand(3) == 0
            end
          end
        end
